...
当访问这个链接时，首先，会根据type为```list```及以上代码去寻找到list.vue的入口组件，在进入list.vue后，后根据第一个参数及本地保存的一个id(5821932ae58a3adc3f8b4642)与应用类型(is_dealer)的映射，然后寻找对应的真正展示的列表vue的组件：dealerList.vue；
...
3、为了提升用户的体验感，登录过程采取自动登录的方式，根据钉钉登录帐号，调用钉钉sdk，与我们自己系统进行匹配，实现自动登录
...
基于以上项目及日常开发，体会到构建一套完整的兼容微信、钉钉、普通页面访问的vue组件库供开发使用的必要性，这样一方面可以在开发新项目时省去重新写新组件或拷贝以前组件的麻烦，另一方面也可以更好、更统一的管理和使用组件库；因此，自己就开始了这个项目，此项目非公司要求，而是自己个人日常开发；
...
在每个组件使用上，尤其反馈操作中，会根据平台信息，使用不同的实现方式；如弹窗提示，当检测到钉钉平台时，会默认调用钉钉sdk提供的弹窗方法。当检测到微信或其他平台时，会默认调用weui提供的弹窗方法实现；并且将weui已有的单选、多选、底部栏、按钮、输入框等都使用vue重新进行了vue组件化的封装，后续将根据需要逐渐增加和完善组件，提供一套可以简单、完整调用的手机开发组件库；
...
为了以后更好维护，我们也对所有页面进行了组件化隔离，使用angular```directive```实现, 每个组件有自己单独的html页面代码：
...
公司核心产品－推事本 web端的开发和维护；主要功能包括任务管理、消息系统、同事、公告、申请、客户管理、文件系统、考勤、项目管理、日历、日报、自定义应用及自定义流程等；在进入公司时，只有任务、消息，在后续的时间里，逐步开发和完善整个功能体系并进行持续维护；

### 项目截图及相关说明

<img alt="web版首页" src="./images/tuishiben_web.png" />
<img alt="web版任务" src="./images/tuishiben_web1.png" />
<img alt="web版消息" src="./images/tuishiben_web2.png" />

上面是功能首页、任务、消息的功能截图，因为最初的架构是使用单页面应用，只有一个入口链接，且使用jquery、 ejs、 less进行开发，没有使用任何框架，所以在后续开发中逐渐暴露出各种问题；

####开发过程中遇到问题
1、在我入职时，整个系统架构已经完成；因此在后续开发功能时，继续延续已开发应用的模式；随着功能的增加，逐渐发觉这种开发方式会造成代码难以维护、代码过于冗余、复用性太低等问题；仅仅任务这个功能的js代码，代码量都超过一万行，再加上ejs中html代码，less中样式代码，代码量非常大，维护起来相当困难；所以在后续功能开发，慢慢去尝试使用框架进行开发，在后续开发中，使用过angular、vue；

2、由于第一个问题，会造成整个系统里有各种的技术选型，开始的jquery，后面的angluar、到现在的vue，所以整个项目复杂度很高；曾经想过通过重构来进行优化，但是经过大家讨论后，不管是从时间、研发成本上来说，对于我们这种初创企业都无法承受，所以直到现在都没有重构；

3、由于属于单页面应用，各个功能之间耦合性太大，所以在每次进页面时，请求资源太多；包括所有需要运行的js、css代码，还有好几个各种初始化的接口请求，所以会导致进页面时间太长，影响用户体验感；后续进行了一些压缩处理，包括js、css、图片、请求接口；也对有些刚进页面不会使用的接口在页面展示出来后不影响用户使用的基础上进行后续请求；

4、在开发过程中，因为系统功能及代码耦合性太大，造成开发某个功能或者修改某个bug时，会产生一些其他原本没有的bug，且开发人员无法把控；所以系统都是计划性上线，每次上线前都会让测试将整个系统功能都测试一遍，再进行上线；

以上就是我在目前公司以来所做的项目展示，对于第一个公司，因为有些项目已经停止运营，所以就没有进行具体描述；

非常感谢您能抽出宝贵的时间了解我的项目经历，如有任何问题及意见建议，可随时与我联系，期待加入你们的队伍；
