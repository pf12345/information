...
公司核心产品－推事本 web端的开发和维护；主要包括任务管理、消息系统、同事、公告、申请、客户管理、文件系统、考勤、项目管理、日历、日报、自定义应用及自定义流程等相关功能；在进入公司时，只有任务、消息功能，在后续的时间里，逐步开发和完善整个功能体系并进行持续维护；
...
####开发过程中遇到问题
1、在我入职时，整个系统架构已经完成；因此在后续开发功能时，一开始继续延续已开发应用的模式进行；随着功能的增加，逐渐发觉这种开发方式会造成代码难以维护、代码过于冗余、复用性太低等问题；仅仅任务这个功能的js代码，代码量都超过一万行，再加上ejs中html代码，less中样式代码，代码量非常大，所以维护起来相当困难；所以在后续功能开发，慢慢去尝试使用框架进行开发，在这几年，使用过angular、vue；

2、由于第一个问题，会造成整个系统里有各种的技术选型，有开始的jquery，后面的angluar、到现在的vue，所以整个项目复杂度很高；曾经想过通过重构来进行优化，但是经过讨论后，不管是从时间、成本上来说，对于我们这种初创企业都无法承受，所以直到现在都是如此；

3、由于属于单页面应用，各个功能之间耦合性太大，所以在每次进页面时，请求资源太多；包括所有需要运行的js、css代码，还有好几个各种初始化的接口请求，所以会导致进页面时间太长，体验感不高；后续进行了一些压缩处理，包括js、css、图片、请求接口；也对有些刚进页面不会使用的接口在页面展示出来后，再不影响用户使用的基础上进行后续请求；

4、在开发过程中，因为系统功能及代码耦合性太大，造成开发某个功能或者修改某个bug时，会产生一些其他原本没有的bug且开发人员无法把控；所以在系统都是计划性上线，每次上线前都会让测试将整个系统功能都测试一遍，再进行上线；
